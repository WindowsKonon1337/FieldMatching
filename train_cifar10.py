...
from src.utils import Config, optimization_manager
...
def main():
    parser = argparse.ArgumentParser(description='EFM Training for CIFAR10')
    ...
    wandb_mode = getattr(config.wandb, 'mode', 'online') if hasattr(config, 'wandb') else 'online'
    wandb.init(
        project=getattr(config.wandb, 'project', 'EFMGenerationCIFAR') if hasattr(config, 'wandb') else 'EFMGenerationCIFAR',
        name=config.wandb.name_exp,
        mode=wandb_mode
    )
    ...
    class EFMWithCheckpoints(EFM):
        def train(self, train_loader, eval_loader, net, optimizer, optimize_fn, state, **kwargs):
            """Переопределяем train для добавления сохранения чекпоинтов"""
    ...
    efm = EFMWithCheckpoints(config, learnable_masses=config.training.learnable_masses)
